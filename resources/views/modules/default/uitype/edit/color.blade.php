<div x-data="{isOpen: false, colors: ['#2196F3', '#009688', '#9C27B0', '#FFEB3B', '#afbbc9', '#4CAF50', '#2d3748', '#f56565', '#ed64a6'], colorSelected: '#FF0000'}" x-cloak>
    <div class="flex items-center">
        <div>
            <input id="category_color" type="text"
                autocomplete="off"
                class="w-full px-4 py-2 mt-2 leading-normal bg-white border border-gray-100 rounded-md shadow"
                x-model="colorSelected">
        </div>
        <div class="relative ml-3">
            <button type="button" x-on:click="isOpen = !isOpen"
                class="inline-flex w-10 h-10 p-2 rounded-full shadow focus:outline-none focus:shadow-outline"
                :style="`background: ${colorSelected || '#c6c6c6'}; color: white`">
            ...
            <div x-show="isOpen" x-on:click.away="isOpen = false" x-transition:enter="transition ease-out duration-100 transform"
                ...
                <div class="px-4 py-3 bg-white rounded-md shadow-xs">
                    <div class="flex flex-wrap -mx-2">
                        <template x-for="(color, index) in colors" :key="index">
                            <div class="px-2">
                                <template x-if="colorSelected === color">
                                    <div class="inline-flex w-8 h-8 border-4 border-white rounded-full cursor-pointer"
                                ...
                                <template x-if="colorSelected != color">
                                    <div
                                        x-on:click="colorSelected = color"
                                        x-on:keydown.enter="colorSelected = color"
                                        role="checkbox"
                                        tabindex="0"
